...
// end
	...
	private static Tensor backward(Module module, Tensor input, Tensor param, Tensor dparam) {
		boolean doparam = (dparam != null);
		// output deriv
		module.forward(input);
		Tensor dout = new Tensor(module.output().size());
		// 1D view
		Tensor sdout = dout.view(dout.nElement());
		// jacobian matrix to calculate
		Tensor jacobian = new Tensor(param.nElement(), dout.nElement());

		for (int i = 1; i <= sdout.nElement(); ++ i) {
			dout.fill(0);
			sdout.set(1, i);
			module.zeroGradParameters();
			Tensor din = module.updateGradInput(input, dout);
			module.accGradParameters(input, dout, 1.0);
			if (doparam) {
				jacobian.select(2, i).copy(dparam);
			}
			else {
	...
	private static Tensor backward(Module module, Tensor input) {
		return backward(module, input, input, null);
	}
...
// end
	...
	private static Tensor backwardUpdate(Module module, Tensor input, Tensor param) {
		// otuput deriv
		module.forward(input);
		Tensor dout = new Tensor(module.forward(input).size());
		// 1D view
		Tensor sdout = dout.view(dout.nElement());
		// jacobian matrix to calculate
		Tensor jacobian = new Tensor(param.nElement(), dout.nElement());
		...
		Tensor[] origparams = new Tensor[params.size()];
		for (int j = 1; j <= params.size(); ++ j) {
			origparams[j - 1] = params.get(j - 1)[0].clone();
		}
		...
		for (int i = 1; i <= sdout.nElement(); ++ i) {
			...
			for (int j = 1; j <= params.size(); ++ j) {
				params.get(j - 1)[0].copy(origparams[j - 1]);
			}
		}
...
// end
	...
	private static Tensor forward(Module module, Tensor input, Tensor param, double perturbation) {
		// 1D view of input
		Tensor sin = param.view(param.nElement());
		// jacobian matrix to calculate
		Tensor jacobian = new Tensor(param.nElement(), module.forward(input).nElement());

		Tensor outa = new Tensor(jacobian.size(2));
		Tensor outb = new Tensor(jacobian.size(2));

		for (int i = 1; i <= sin.nElement(); ++ i) {
			double orig = sin.get(i);
			sin.set(orig - perturbation, i);
			outa.copy(module.forward(input));
			sin.set(orig + perturbation, i);
			outb.copy(module.forward(input));
			sin.set(orig, i);

			add(outb, outb, -1, outa);
			div(outb, outb, 2 * perturbation);
			jacobian.select(1, i).copy(outb);
		}
	...
	private static Tensor forward(Module module, Tensor input) {
		return forward(module, input, input, 1e-6);
	}
...
// end
	...
	private static Tensor forwardUpdate(Module module, Tensor input, Tensor param, double perturbation) {
		// 1D view of input
		Tensor sin = param.view(param.nElement());
		// jacobian matrix to calculate
		Tensor jacobian = new Tensor(param.nElement(), module.forward(input).nElement());
		...
		for (int i = 1; i <= sin.nElement(); ++ i) {
			...
			div(outb, outb, 2 * perturbation);
			jacobian.select(1, i).copy(outb);
			mul(jacobian.select(1, i), jacobian.select(1, i), -1);
			add(jacobian.select(1, i), jacobian.select(1, i), sin.get(i));
		}

		return jacobian;
	}

	private static Tensor forwardUpdate(Module module, Tensor input, Tensor param) {
		return forward(module, input, param, 1e-6);
	}
...
// end
	...
	public static double testJacobian(Module module, Tensor input, double minval, double maxval, double perturbation) {
		uniform(input, minval, maxval, input.size());
		Tensor jac_fprop = forward(module, input, input, perturbation);
		Tensor jac_bprop = backward(module, input);
		Tensor error = csub(jac_fprop, jac_bprop);
		return max(abs(error));
	}

	public static double testJacobian(Module module, Tensor input) {
		return testJacobian(module, input, -2, 2, 1e-6);
	}
...
// end
	...
	public static double testJacobianParameters(Module module, Tensor input, Tensor param, Tensor dparam, double minval, double maxval, double perturbation) {
		uniform(input, minval, maxval, input.size());
		uniform(param, minval, maxval, param.size());
		Tensor jac_bprop = backward(module, input, param, dparam);
		Tensor jac_fprop = forward(module, input, param, perturbation);
		Tensor error = csub(jac_fprop, jac_bprop);
		return max(abs(error));
	}

	public static double testJacobianParameters(Module module, Tensor input, Tensor param, Tensor dparam) {
		return testJacobianParameters(module, input, param, dparam, -2, 2, 1e-6);
	}
...
// end
	...
	public static double testJacobianUpdateParameters(Module module, Tensor input, Tensor param, double minval, double maxval, double perturbation) {
		uniform(input, minval, maxval, input.size());
		uniform(param, minval, maxval, param.size());
		Tensor jac_bprop = backwardUpdate(module, input, param);
		Tensor jac_fprop = forwardUpdate(module, input, param, perturbation);
		Tensor error = csub(jac_fprop, jac_bprop);
		return max(abs(error));
	}

	public static double testJacobianUpdateParameters(Module module, Tensor input, Tensor param) {
		return testJacobianUpdateParameters(module, input, param, -2, 2, 1e-6);
	}
