...
public class DepthConcat extends Concat
{

	public DepthConcat(int outputDimension) {
		super(outputDimension);
	}
	
	@Override
	public Module clone() {
		DepthConcat clone = new DepthConcat(outputDimension);
...
// end
	...
	Tensor _windowNarrow(Tensor output, Tensor currentOutput, int offset) {
		Tensor outputWindow = output.narrow(outputDimension, offset, currentOutput.size(outputDimension));
		for (int dim = 1; dim <= size.length; ++ dim) {
			int currentSize = currentOutput.size(dim);
			if (dim != outputDimension && size[dim - 1] != currentSize) {
				int start = (int)Math.floor((size[dim - 1] - currentSize) / 2.0 + 1);
				outputWindow = outputWindow.narrow(dim, start, currentSize);
			}
		}
		return outputWindow;
	...
	Tensor _getOutput(Tensor gradOutput, Tensor currentOutput, int offset) {
		return _windowNarrow(gradOutput, currentOutput, offset);
	}
...
// end
	...
	void _updateSize(int i, Tensor currentOutput) {
		if (i == 1) {
			...
			for (int dim = 1; dim <= size.length; ++ dim) {
				if (dim == outputDimension) continue;
				if (size[dim - 1] >= currentOutput.size(dim)) continue;
				size[dim - 1] = currentOutput.size(dim);
			}
		}
	}
